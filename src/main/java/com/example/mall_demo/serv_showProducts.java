...
public class serv_showProducts extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                htmlBuilder.append("<button onclick=\"toggleText(").append(id).append(", '").append(description).append("')\">点击展示/隐藏详细信息</button>");
                htmlBuilder.append("<div id=\"textArea").append(id).append("\" style=\"display: none;\">");
                htmlBuilder.append("<p>").append(description).append("</p>");
                htmlBuilder.append("</div>");
                htmlBuilder.append("<form action=\"addToCart\" method=\"post\">");
                htmlBuilder.append("<input type=\"hidden\" name=\"productId\" value=\"").append(id).append("\">");
                htmlBuilder.append("<button type=\"submit\">添加至购物篮</button>");
